  ...
  <div>
    <div class="row page-titles">
      <div class="col-md-5 align-self-center">
        <h3 class="text-themecolor">{{ trans(`users.${pageTitle}`) }}</h3>
      </div>
              ...
              <div class="card-body">
                <el-tabs>
                  <el-tab-pane :label="trans('users.tabs.data')">
                    ...
                    <el-form-item
                      :label="trans('users.form.first-name')"
                      :class="{
                    ...
                    <el-form-item
                      :label="trans('users.form.last-name')"
                      :class="{
                    ...
                    <el-form-item
                      :label="trans('users.form.is activated')"
                      :class="{
                        'el-form-item is-error': form.errors.has('activated'),
                      }"
                    >
                      <el-checkbox v-model="user.is_activated"
                        >Activated</el-checkbox
                      >
                      <div
                        class="el-form-item__error"
                        v-if="form.errors.has('activated')"
                        v-text="form.errors.first('activated')"
                      ></div>
                    </el-form-item>
                  ...
                  >
                    <div v-if="!user.is_new">
                      ...
                      <div class="col-md-6">
                        <h4>
                          {{ trans("users.tabs.or send reset password mail") }}
                        </h4>
                        <el-button
                          type="info"
    ...
    <button
      v-shortkey="['b']"
      @shortkey="pushRoute({ name: 'admin.user.users.index' })"
      v-show="false"
    ></button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetchRoles() {
      axios.get(route("api.user.role.all")).then((response) => {
    ...
    sendResetEmail() {
      ...
      axios
        .get(
          route("api.user.user.sendResetPassword", {
            user: this.$route.params.userId,
          })
        )
        .then((response) => {
